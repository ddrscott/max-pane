//! Paste history's table (ADR-0031): what is kept, what never is, caps,
//! ageing, deletion, and that none of it leaves in a strip export.
...
fn pastes_and_copies_are_listed_newest_first() {
    ...
    assert!(core.record_clip(pane.clone(), ClipKind::Paste, "/tmp/a path".into(), 200, 30).unwrap());
    assert!(core.record_clip(pane.clone(), ClipKind::Copy, "one\ntwo\n".into(), 200, 30).unwrap());
...
fn the_same_text_again_moves_to_the_top() {
    ...
    let pane = terminal(&core);
    for text in ["a", "b", "a"] {
        core.record_clip(pane.clone(), ClipKind::Paste, text.into(), 200, 30).unwrap();
    }
...
fn a_private_lane_and_an_unknown_pane_record_nothing() {
    ...
    assert!(!core.record_clip(private, ClipKind::Paste, "in private".into(), 200, 30).unwrap());
    assert!(!core.record_clip("no-such-pane".into(), ClipKind::Copy, "from nowhere".into(), 200, 30).unwrap());
    assert!(contents(&core).is_empty());
    // The same call from an ordinary lane records: the refusal is the lane's.
    let pane = terminal(&core);
    assert!(core.record_clip(pane, ClipKind::Paste, "in public".into(), 200, 30).unwrap());
...
fn blank_text_too_much_text_and_keep_zero_record_nothing() {
    let core = Core::open_in_memory().unwrap();
    let pane = terminal(&core);
    assert!(!core.record_clip(pane.clone(), ClipKind::Paste, " \n".into(), 200, 30).unwrap());
    assert!(!core.record_clip(pane.clone(), ClipKind::Paste, "x".repeat(64 * 1024 + 1), 200, 30).unwrap());
    assert!(core.record_clip(pane.clone(), ClipKind::Paste, "x".repeat(64 * 1024), 200, 30).unwrap());
    assert!(!core.record_clip(pane, ClipKind::Paste, "off".into(), 0, 30).unwrap());
...
fn a_secret_is_stored_as_four_characters_and_never_as_itself() {
    ...
    let secret = "export KEY=sk-ant-api03-abcdefghijklmnopqrstuvwx";
    {
        ...
        let pane = terminal(&core);
        assert!(core.record_clip(pane, ClipKind::Paste, secret.into(), 200, 30).unwrap());
        let all = core.clip_history(200, 30).unwrap();
...
fn the_cap_keeps_the_newest_and_the_setting_is_read_each_time() {
    ...
    let pane = terminal(&core);
    for i in 0..10 {
        core.record_clip(pane.clone(), ClipKind::Paste, format!("n{i}"), 5, 30).unwrap();
    }
...
fn entries_age_out_and_zero_days_keeps_any_age() {
    ...
    let now = 100 * DAY;
    assert!(ledger.record_clip(&pane, ClipKind::Paste, "old", 200, 0, now - 31 * DAY).unwrap());
    assert!(ledger.record_clip(&pane, ClipKind::Paste, "recent", 200, 0, now - 29 * DAY).unwrap());
    ...
    assert!(ledger.record_clip(&pane, ClipKind::Copy, "new", 200, 30, now + 2 * DAY).unwrap());
    let left: Vec<String> = ledger.clips().unwrap().into_iter().map(|c| c.content).collect();
    assert_eq!(left, ["new"]);
}
...
fn delete_removes_one_and_clear_removes_all() {
    ...
    let pane = terminal(&core);
    for text in ["a", "b", "c"] {
        core.record_clip(pane.clone(), ClipKind::Copy, text.into(), 200, 30).unwrap();
    }
...
fn a_strip_export_has_no_paste_history_in_it() {
    let core = Core::open_in_memory().unwrap();
    let pane = terminal(&core);
    core.record_clip(pane, ClipKind::Paste, "only-in-paste-history".into(), 200, 30).unwrap();
