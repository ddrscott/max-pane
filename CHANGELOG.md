...
### Added
- **A terminal palette of your own**, and a font size that changes while you watch. `terminal_theme_dark` and `terminal_theme_light` in `config.toml` name one of the **485 Ghostty themes** libghostty already ships — the table Ghostty itself picks from, enumerated rather than copied, so a library update adds themes here. Two keys and not an inline table, because the file is a flat list and `[keys]` is the one nesting it earned. The name is matched without regard to case and written back as the table spells it; a name that is not a theme keeps the default and says so with a near miss to try (`"dracul"` → `such as "Dracula"`). Three colours stay the app's whatever is named — the lane background so there is no seam under the header, the accent cursor, and the faint accent selection wash over text that keeps its own colour — because ADR-0009 puts the palette in the *theme*, where the last line for a key wins. Unset renders exactly what it rendered before. **Live, measured:** `ghostty_surface_update_config` reaches every surface already on the strip, so a terminal repaints in the new palette without a rebuild and without losing what is on it; a real-surface test captures a still terminal, names a theme, and watches the pixels change with the grid unmoved. `font_name`, `font_size`, `copy_on_select` and `cursor_blink` ride the same push and now say **live** rather than "relaunch to apply". Pick one in **Settings › Appearance** — a name field and a `pick…` menu with every theme under its initial — or in **⌘E**, where `THEMES` is a header and one worn-pair line until you type part of a name. **⌘=** / **⌘-** / **⌘0** size one terminal's text on the web pane's own ladder, per pane and in the ledger, and now step from whatever `font_size` currently says (ADR-0043)
- A chord per web app: `[[apps]]` in `config.toml` — `name`, `url`, an optional `key` and an optional `docked = "left" | "right"` — and the chord **focuses the lane that app is already on**, opening one only when there is none, so pressing it twice is pressing it once. "Already on" is by registrable domain, the ad blocker's own eTLD+1 rule and not a second one, so a redirect and a message you clicked into are still Gmail's lane; several lanes on the site means the most recently focused. `docked` applies only when the chord has to open one — a lane already up is focused where it is. The keymap now has a **second source**, laid over `[keys]` rather than under it: a chord a command holds, macOS owns, or an earlier `[[apps]]` table took is **refused and named** (`⌘G: the app Gallery does not get it — toggleGallery has it`), costing the app its key and nothing else, because a command's chord is in the menu bar and on ⌘/ and an app's is in neither. A page never sees an app's ⌘-chord, for the same reason it never sees ⌘O. `// APPS` in ⌘/, **Settings › Apps** with the same `rec` / `none` recorder the Keyboard section has, rows in ⌘E saying whether ↩ will `focus` or `open` with ⌘⌫ to bind, and `maxpane app gmail` from a shell. Names, addresses and `docked` apply at once; the chord is read at launch and says `relaunch to apply` (ADR-0042)
- ⇧⌘H Paste History now keeps a **⌘C made in a web pane**: copy a token off a dashboard, copy something else, and the token is still there. The copy is WebKit's own — the pane performs it and keeps what it put on the clipboard for the fifth of a second that takes — so nothing a *page* copies for you (`navigator.clipboard.writeText`, a page's copy button, WebKit's right-click Copy), nothing in a sign-in popup and nothing copied in another app is ever recorded; there is still no timer anywhere near the clipboard. A picture copied that way becomes `copy-….png` beside the pasted ones, swept by the same `paste_image_keep_days`, and the row is its path, so ↩ hands an agent a picture you copied off a page. Private lanes, the three nspasteboard.org marks, 64 KB and the secret shapes refuse a web pane exactly as they refuse a terminal — the ledger decides, not the pane. Every row now wears a grey `PTY` / `WEB` chip (ADR-0041)
