...
### Fixed
- Pasting into the ⌘O picker went to the terminal behind it, and the same went for any picker, sheet, popover or confirmation in front: ⌘V, ⌘C and the Paste Special items reached past the dialog to the pane. A dialog in front now owns the keyboard, and strip commands grey out in the menu while it is up, so ⌘W with the picker open no longer closes the lane behind it
